...
class RegisterForm(forms.ModelForm):
    ...
    password = forms.CharField(label='Pasword', max_length=100, required=True)
    password_confirm = forms.CharField(label='Confirm Pasword', max_length=100, required=True)
...
class ForgotPasswordForm(forms.Form): 
    ...
    email = forms.EmailField(label = 'Email', max_length=254, required=True) #email field for the user to enter their email address

    def clean(self):  #method used for custom valiadation
        cleaned_data = super().clean()   #get cleaned data 
        email = cleaned_data.get('email')  #extract the email entered by user         
        # check if the email exits in database
        if not User.objects.filter(email=email).exists():
            raise forms.ValidationError("No User Regsiter with this email") #raise an validation error if the email is not found 
